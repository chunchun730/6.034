...
def path_length(graph, path):
    ...
    p = path[:]
    while len(p) > 1:
        length += graph.get_edge(p[0], p[1]).length
        p.pop(0)
    return length
...
def extensions(graph, path):
    ...
    new_path = []
    for n in neighbors:
        if not has_loops(path_copy+[n]):
            new_path.append(path_copy+[n])
    new_path.sort()
    return new_path
...
def sort_by_heuristic(graph, goalNode, nodes):
    ...
    node_h = [(n, graph.get_heuristic_value(n, goalNode)) for n in nodes]
    return [ i[0] for i in sorted(sorted(node_h, key=lambda x: x[0]), key = lambda y: y[1])]
...
def h_c_sort_new_paths_fn(graph, goalNode, paths):
    return sorted(sorted(paths), key=(lambda path: graph.get_heuristic_value(path[-1], goalNode)) )

def b_f_sort_agenda_fn(graph, goalNode, agenda_paths):
    return sorted(sorted(agenda_paths), key=(lambda path: graph.get_heuristic_value(path[-1], goalNode)) )

def b_b_sort_agenda_fn(graph, goalNode, agenda_paths):
    return sorted(sorted(agenda_paths), key=(lambda path: path_length(graph, path)))

def b_b_h_sort_agenda_fn(graph, goalNode, agenda_paths):
    return sorted(sorted(agenda_paths), key=(lambda path: path_length(graph, path) + graph.get_heuristic_value(path[-1], goalNode)) )
...
generic_hill_climbing = [h_c_sort_new_paths_fn, True, do_nothing_fn, False]

generic_best_first = [do_nothing_fn, True, b_f_sort_agenda_fn, False]

generic_branch_and_bound = [do_nothing_fn, False, b_b_sort_agenda_fn, False]

generic_branch_and_bound_with_heuristic = [do_nothing_fn, False, b_b_h_sort_agenda_fn, False]

generic_branch_and_bound_with_extended_set = [do_nothing_fn, False, b_b_sort_agenda_fn, True]

generic_a_star = [do_nothing_fn, False, b_b_h_sort_agenda_fn, True]
...
def my_beam_sorting_fn(graph, goalNode, agenda_paths, beam_width):
    if len(agenda_paths) > 0 and not all(len(p) == len(agenda_paths[0]) for p in agenda_paths):
        return agenda_paths
    s = sorted(sorted(agenda_paths), key=(lambda path: graph.get_heuristic_value(path[-1], goalNode)) )[:beam_width]
    return s
...
def is_admissible(graph, goalNode):
    ...
    nodes = graph.nodes
    for node in nodes:
        path = branch_and_bound(graph, node, goalNode)
        for i in range(len(path)):
            if graph.get_heuristic_value(path[i], path[-1]) > path_length(graph, path[i:]):
                return False
    return True
...
def is_consistent(graph, goalNode):
    ...
    for node in nodes:
        ...
        for i in range(len(path)-1):
            diff = abs(graph.get_heuristic_value(path[i], path[-1]) - graph.get_heuristic_value(path[i+1], path[-1]))
            if diff > graph.get_edge(path[i], path[i+1]).length:
                return False
    return True
...
[h2_S, h2_A, h2_B, h2_C, h2_G] = [9, 8, 9, 5, 0]
...
[h3_S, h3_A, h3_B, h3_C, h3_G] = [9, 8, 5, 1, 0]
...
[h4_S, h4_A, h4_B, h4_C, h4_G] = [9, 8, 8, 1, 0]
...
WHAT_I_FOUND_INTERESTING = 'Implementing sorting'
WHAT_I_FOUND_BORING = 'Long reading and still unclear explanation'
SUGGESTIONS = 'None'
